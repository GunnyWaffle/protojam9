...
public class EnemyFormationSpawner : MonoBehaviour {
    ...
    public static EnemyFormationSpawner instance;

    public List<GameObject> typesOfFormations = new List<GameObject>();
    public int numSquadsOnScreen;
    ...
    private void Update()
    {
        if (squadsOnScreen < numSquadsOnScreen && lastSpawn <= 0.0f && shouldSpawn)
        {
            SpawnEnemy();
    ...
    void SpawnEnemy()
    {
        ...
        int enemyType = Random.Range(0, typesOfFormations.Count);
        GameObject squad = typesOfFormations[enemyType];
        float startingAngle;
        ...
        GameObject newEnemy = Instantiate(squad, spawnLocation, Quaternion.Euler(0, 180, 0));
        // May need to init the enemy
        squadsOnScreen++;
    }

    public void KilledSquad()
    {
        squadsOnScreen--;
        if (squadsOnScreen < 0)
            squadsOnScreen = 0;
